---
type: concept
title: "自动驾驶"
date: 2026-06-27
updated: 2026-06-27
tags:
  - automotive
  - adas
  - perception
  - automotive-eea
source_count: 4
confidence: low
domain_volatility: medium
last_reviewed: 2026-06-27
aliases:
  - "自动驾驶"
  - "智能驾驶"
  - "Autonomous Driving"
  - "autonomous-driving"
---

# 自动驾驶（Autonomous Driving）

## Definition

自动驾驶（Autonomous Driving）指车辆通过环境感知、决策规划与控制执行实现不同程度的自主行驶。环境感知依赖摄像头、激光雷达（见 [[lidar]]）、毫米波雷达（见 [[mmwave-radar]]）等多传感器融合，是汽车传感器（见 [[automotive-sensor]]）需求增长的核心驱动力。

## Key Points

- 三大环节：环境感知 → 决策规划 → 控制执行
- 感知依赖多传感器融合：摄像头 + 激光雷达 + 毫米波雷达
- 是汽车传感器（见 [[automotive-sensor]]）市场增长的核心驱动
- 传感器配置随自动驾驶等级提升而增加

## My Position

<!-- 暂无个人立场，待补充 -->

## Contradictions

<!-- 暂未发现来源间分歧 -->

## Sources

- [[sources/kpmg-automotive-sensor-market-2024]]
- [[sources/jiushi-autonomous-vehicle-brain-hardware]]
- [[sources/neolix-x3-plus-compute-hardware]]
- [[sources/whiterhino-rx-compute-unit]]

## Evolution Log

- 2026-06-27（1 sources）：概念初建，从芯片设计/制造来源批量提取，消除 source 页断链
- 2026-07-21（1 sources）：REFLECT 补齐主域标签：automotive-eea
- 2026-08-28（4 sources）：强化——九识智能（双 Orin X，L4 VLA）、新石器 X3 Plus（单 DRIVE Orin-X 254 TOPS ASIL-D）、白犀牛 RX（经纬恒润域控 TwoBox 冗余制动）三款国产 L4 无人配送车算力方案；详见 [[robot-brain-compute-platform]]
